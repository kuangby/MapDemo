...
ChunkManager::GridLines ChunkManager::getGridLines(int centerChunkX, int centerChunkZ, int radiusChunks) {
    ...
    for (int z = minZ; z <= maxZ + 1; ++z) {
        result.horizontal.push_back({static_cast<float>(z * 16), {static_cast<float>(minX * 16), static_cast<float>((maxX + 1) * 16)}});
    }

    // 竖线：沿 z 方向，x = minX .. maxX+1
    for (int x = minX; x <= maxX + 1; ++x) {
        result.vertical.push_back({static_cast<float>(x * 16), {static_cast<float>(minZ * 16), static_cast<float>((maxZ + 1) * 16)}});
    }
