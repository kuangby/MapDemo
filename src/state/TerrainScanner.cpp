...
bool isBlockOpaque(const std::string& name, int dim) {
    ...
    if (dim == 0) { // 主世界
        if (name == "minecraft:stone" || name == "minecraft:deepslate" ||
            name == "minecraft:grass_block" || name == "minecraft:dirt" ||
...
BlockColor processWater(
    LevelChunk* chunk, int cx, int cz, int waterSurfaceY, int minY,
...
BlockColor getBlockColorAtCameraHeight(LevelChunk* chunk, int cx, int cz, int cameraHeight, int minY, int maxY, int dim) {
    ...
    auto getColor = [&](const std::string& name) -> BlockColor {
        return getBlockColorWithBiome(name, biomeName);
    };
...
void TerrainScanner::scanChunk(BlockSource* region, const ScanChunkKey& key, int minY, int maxY) {
    ...
    int cameraHeight;
    switch (key.dim) {
        case 0: cameraHeight = config::getConfig().terrain.cameraHeight.overworld; break;
        case 1: cameraHeight = config::getConfig().terrain.cameraHeight.nether; break;
        case 2: cameraHeight = config::getConfig().terrain.cameraHeight.end; break;
        default: cameraHeight = config::getConfig().terrain.cameraHeight.overworld; break;
    }
