    ...
    if (ShadowDebugLogger::getInstance().isEnabled()) {
        ...
        for (auto& row : handlingRegion) {
            for (auto& chunk : row) {
                if (!chunk) continue;
                ++bakedChunks;
                for (auto& brow : chunk->blocksData) {
                    for (auto& block : brow) {
        ...
        ShadowDebugLogger::getInstance().log(
            "[bakeTrace] region=({},{}) dim={} style={} scale={} chunks={} heightQueries={} misses={}"
            " shadowedSubpixels={}",
            handlingRegionPos.x,
            handlingRegionPos.z,
    ...
    if (sunX <= 0 && sunZ <= 0) {
        for (int chunkZ = 0; chunkZ < 16; chunkZ++) {
            ...
            for (int chunkX = 0; chunkX < 16; chunkX++) {
                ...
                if (ShadowDebugLogger::getInstance().isEnabled()
                    && ShadowDebugLogger::hasWatchedBlockInChunk(
                        handlingRegionPos.x * 16 + chunkX,
                        handlingRegionPos.z * 16 + chunkZ
                    )) {
                    ...
                    ShadowDebugLogger::getInstance().log(
                        "[shadowMap] region=({},{}) chunk=({},{}) dim={} scale={} path=FAST west={} northWest={}"
                        " north={}",
                        handlingRegionPos.x,
                        handlingRegionPos.z,
                        handlingRegionPos.x * 16 + chunkX,
                        ...
                        dimId,
                        scale,
                        neighborState(chunkX - 1, chunkZ),
                        neighborState(chunkX - 1, chunkZ - 1),
                        neighborState(chunkX, chunkZ - 1)
                    );
                }
                for (int blockZ = 0; blockZ < 16; blockZ++) {
                    ...
                    for (int blockX = 0; blockX < 16; blockX++) {
                        ...
                        int  worldBX = handlingRegionPos.x * 256 + chunkX * 16 + blockX;
                        int  worldBZ = handlingRegionPos.z * 256 + chunkZ * 16 + blockZ;
                                    ...
                                    if (sx != lastOffsetPos.x || sz != lastOffsetPos.z) {
                                        ...
                                        if (watched) {
                                            ShadowDebugLogger::getInstance().log(
                                                ...
                                                scaleZ,
                                                s,
                                                handlingRegionPos.x * 256 + sx,
                                                handlingRegionPos.z * 256 + sz,
                                                sampleHeight,
                                                currentHeight
                                            );
                                        }
                    ...
                    for (int blockX = 0; blockX < 16; blockX++) {
                        ...
                        if (ShadowDebugLogger::getInstance().isEnabled()
                            && ShadowDebugLogger::isWatchedBlock(
                                handlingRegionPos.x * 256 + chunkX * 16 + blockX,
                                handlingRegionPos.z * 256 + chunkZ * 16 + blockZ
                            )) {
                            ShadowDebugLogger::getInstance().log(
